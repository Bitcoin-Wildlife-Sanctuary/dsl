...
impl M31Mult {
    pub fn compute_c_limbs_from_limbs(a_limbs: &[u32], b_limbs: &[u32]) -> Result<[u32; 4]> {
        ...
        c_limbs[0] += u32::try_from(a_limbs[0] * b_limbs[0])?;

        c_limbs[1] += u32::try_from(a_limbs[0] * b_limbs[1])?;
        c_limbs[1] += u32::try_from(a_limbs[1] * b_limbs[0])?;

        c_limbs[2] += u32::try_from(a_limbs[0] * b_limbs[2])?;
        c_limbs[2] += u32::try_from(a_limbs[1] * b_limbs[1])?;
        c_limbs[2] += u32::try_from(a_limbs[2] * b_limbs[0])?;

        c_limbs[3] += u32::try_from(a_limbs[0] * b_limbs[3])?;
        c_limbs[3] += u32::try_from(a_limbs[1] * b_limbs[2])?;
        c_limbs[3] += u32::try_from(a_limbs[2] * b_limbs[1])?;
        c_limbs[3] += u32::try_from(a_limbs[3] * b_limbs[0])?;

        c_limbs[0] += u32::try_from(a_limbs[1] * b_limbs[3])?
            .checked_shl(1)
            .ok_or(Error::msg("Unexpected overflow"))?;
        c_limbs[0] += u32::try_from(a_limbs[2] * b_limbs[2])?
            .checked_shl(1)
            .ok_or(Error::msg("Unexpected overflow"))?;
        c_limbs[0] += u32::try_from(a_limbs[3] * b_limbs[1])?
            .checked_shl(1)
            .ok_or(Error::msg("Unexpected overflow"))?;

        c_limbs[1] += u32::try_from(a_limbs[2] * b_limbs[3])?
            .checked_shl(1)
            .ok_or(Error::msg("Unexpected overflow"))?;
        c_limbs[1] += u32::try_from(a_limbs[3] * b_limbs[2])?
            .checked_shl(1)
            .ok_or(Error::msg("Unexpected overflow"))?;

        c_limbs[2] += u32::try_from(a_limbs[3] * b_limbs[3])?
            .checked_shl(1)
            .ok_or(Error::msg("Unexpected overflow"))?;
